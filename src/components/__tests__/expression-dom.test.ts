...
/** Mirrors `makeChip`'s `render` arm: same shape as `varChip`, distinct sigil. */
const renderChip = (ref: string) =>
  el("span", { "data-kind": "render", "data-ref": ref }, [
    text("▤"),
    text(ref.startsWith("vars.") ? ref.slice("vars.".length) : ref),
    text("×"),
  ]);
...
test("readParts reads a render chip's data-kind/data-ref back verbatim", () => {
  const parts = read([text("Body: "), renderChip("documents.a.body")]);
  assert.deepEqual(parts, [
